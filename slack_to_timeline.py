...
def send_dm():
    print("Received /dm request", flush=True)
    ...
    if token != VERIFICATION_TOKEN:
        print("Invalid request token", flush=True)
        return jsonify({"error": "Invalid request token"}), 403

    if user_id not in AUTHORIZED_USERS:
        print("Unauthorized user", flush=True)
        return jsonify({"error": "Unauthorized user"}), 403

    channel_id = data.get('channel_id')
    if not channel_id:
        print("channel_id is required", flush=True)
    ...
    print("Opening direct message channel", flush=True)
    ...
    if response.status_code != 200 or not response.json().get('ok'):
        ...
        print(f"Error opening direct message channel: {error_message}", flush=True)
    ...
    print("Sending message to direct message channel", flush=True)
    ...
    if response.status_code != 200 or not response.json().get('ok'):
        ...
        print(f"Error sending message: {error_message}", flush=True)
        return jsonify({"error": error_message}), response.status_code
...
def get_history(channel):
    print(f"Fetching history for channel: {channel}", flush=True)
    ...
    print(f"Verified user ID: {user_id}", flush=True)
    if user_id not in AUTHORIZED_USERS:
        print("Unauthorized user", flush=True)
    ...
    print(f"Fetching conversation history with params: {params}", flush=True)
...
def download_file(chat_id):
    print(f"Downloading file for chat_id: {chat_id}", flush=True)
    try:
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        ...
        print(f"Error downloading file: {str(e)}", flush=True)
...
async def conversion(chat_id):
    print(f"Starting conversion for chat_id: {chat_id}", flush=True)
    ...
    async def formatJSON(file_path, exportdata, output_file_path, slack_token):
        print(f"Formatting JSON for file: {file_path}", flush=True)
        with open(file_path, 'r') as file:
            importdata = json.load(file)
            
        array = importdata.get('messages', {})
        print(f"Number of messages: {len(array)}", flush=True)
        ...
        print(f"First date: {first_date}, Last date: {last_date}", flush=True)
        ...
        print(f"Processing {len(tasks)} messages", flush=True)
    ...
    async def get_user_info(slack_token, user_id):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url, headers=headers, params=params) as response:
                ...
                print(f"User info for {user_id}: {data}", flush=True)
    ...
    print(f"Formatting JSON for chat_id: {chat_id}", flush=True)
    formatted_data = await formatJSON(file_path, exportdata, output_file_path, SLACK_TOKEN)
    print(f"Formatted data for chat_id: {chat_id}", flush=True)
